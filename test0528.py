...
from sklearn.metrics import roc_auc_score  # 改为AUC评估
import joblib

def collate_classification(batch):  # 修改collate函数名称
...
def train_classification(CDS_sequences, Five_UTR_sequences, Three_UTR_sequences, labels, k, word_vectors_path, epochs,
                         num_classes=2, resume_from_checkpoint=None):
    batch_size = 16
    model_path = "result/best_bert_cls.pth"  # 修改模型保存名称
    result_save_path = f"result/BERT_cls_ep{epochs}.txt"
    ...
    train_dataset = BERTDataset(
        ...
        k, word_vectors_path,
        task_type="classification",  # 改为分类任务
    ...
    if resume_from_checkpoint:
        ...
        best_loss = checkpoint.get('best_loss', checkpoint['loss'])  # 兼容旧检查点
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch in train_loader:
            ...
            optimizer.zero_grad()
            outputs = model(word_embeds, segment_ids, task_type='classification')  # 指定分类任务
        ...
        if (epoch + 1) % 20 == 0:
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    probs = torch.softmax(outputs, dim=1).cpu().numpy()  # 获取概率
            ...
            torch.save(checkpoint, f"result/checkpoint_cls_epoch_{epoch + 1}.pth")
            result_file.write(f"Checkpoint saved: checkpoint_cls_epoch_{epoch + 1}.pth\n")
            result_file.write(f"The AUC at the checkpoint_cls_epoch_{epoch + 1} is {auc_score}\n")
            result_file.flush()
...
train_classification(CDS_sequences, Five_UTR_sequences, Three_UTR_sequences,
